...
(function () {
    ...
    */
    ...
    OeuvreAPI.prototype.routes = function(app) {
        ...
        // route v0.1.0
        app.get("/oeuvre", this.liste);
        app.get("/oeuvre/liste", this.liste);
        app.get("/oeuvre/liste/:pageSize/:offset", this.listePaged);
        app.get("/oeuvre/recent", this.recentlyUpdated);
        app.get("/oeuvre/compte", this.compte);
        app.get("/oeuvre/detail/:id", this.detail);

        app.post("/oeuvre", this.ajout);
        app.put("/oeuvre", this.update);
        app.patch("/oeuvre", this.undef); // Verrouillage Todo
        app.delete("/oeuvre/:oeuvreId", this.delete);

    };

    //retourne les details d'une oeuvre
    OeuvreAPI.prototype.detail = function(req, res){
        oeuvreDAL.get(req.params.id, function(data){
            res.send(data);
        });
    };
